...
#+begin_src emacs-lisp
  ...
  (defun context-insert-tag-pair (start end)
    ...
    (insert start)
    (with-save-point (newline) (insert end)))
  (evil-define-operator context-wrap-tag-pair (beg end)
    ...
    (let ((pair (context-get-tag-pair)))
      ...
      (goto-char beg)
      (insert (car pair))
      (with-save-point (newline))))

  (setf context-start/end-tags '("start/stop" "b/e"))
  (setf context-tags (nconc
                         '("paragraph" "itemize" "component" "TABLE" "TABLEhead" "TABLEbody" "TC" "TR")
                         (cl-loop for i in '("section" "subject")
                                  nconc (mapcar
                                         (lambda (subs) (concat subs i))
                                         '("" "sub" "subsub" "subsubsub" "subsubsubsub")))))
