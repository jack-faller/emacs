...
#+begin_src emacs-lisp
  ...
  (evil-define-key 'insert corfu-map
    ...
    (kbd "M-TAB") #'corfu-next
    (kbd "M-<tab>") #'corfu-quit
...
#+begin_src emacs-lisp
  (evil-define-key 'insert corfu-map
    (kbd ".") (evil-define-command corfu-insert-ret () (corfu-insert) (insert "."))
    (kbd "RET") (evil-define-command corfu-insert-ret () (corfu-insert) (insert "\n"))
    ...
    (kbd "M-q") (evil-define-command corfu-0th () (corfu-first) (corfu-previous))
    (kbd "/") (evil-define-command corfu-/ ()
                (corfu-insert) (completion-at-point)))
  ;; / is for file names but unbind it in lisp modes as it would conflict with symbol bindings.
  (hook '(lisp-mode-hook emacs-lisp-mode-hook scheme-mode-hook)
    (evil-define-key 'insert corfu-map (kbd "/") nil))
#+end_src
*** Prettify
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (mapc (lambda (x) (add-to-list 'completion-at-point-functions x))
        (list #'cape-file #'cape-dabbrev #'cape-keyword #'cape-tex #'cape-sgml #'cape-abbrev))
#+end_src
*** Documentation
#+begin_src emacs-lisp
  (pkg corfu-doc :require t)
  (add-hook 'corfu-mode-hook #'corfu-doc-mode)
  (setf corfu-doc-max-width 80)
  (evil-define-key nil corfu-map
    (kbd "M-d") #'corfu-doc-scroll-down
    (kbd "M-u") #'corfu-doc-scroll-up)
#+end_src
** Eldoc
#+begin_src emacs-lisp
  (diminish 'eldoc-mode)
